...
class MazeEnv(mujoco_env.MujocoEnv, utils.EzPickle, offline_env.OfflineEnv):
    def __init__(self,
                 ...
                 with_target= False,
                 **kwargs):
        # d4rl/gym variants differ in the kwargs accepted by OfflineEnv.
        # Forwarding unknown kwargs can hit object.__init__(**kwargs) and crash.
        offline_keys = {
            "dataset_url",
            "ref_max_score",
            "ref_min_score",
        ...
        offline_kwargs = {k: kwargs.pop(k) for k in list(kwargs.keys()) if k in offline_keys}
        try:
            offline_env.OfflineEnv.__init__(self, **offline_kwargs)
        except TypeError:
            offline_env.OfflineEnv.__init__(self)
